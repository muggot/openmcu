...
VideoMixPosition::VideoMixPosition(ConferenceMemberId _id,  int _x, int _y, int _w, int _h)
...
void MCUVideoMixer::LeftSplitLine(BYTE *d, unsigned px, unsigned py, unsigned pw, unsigned ph, unsigned width, unsigned height)
{
  unsigned int i, p;
  for(i = py; i < py+ph; i++)
  {
...
void MCUVideoMixer::RightSplitLine(BYTE *d, unsigned px, unsigned py, unsigned pw, unsigned ph, unsigned width, unsigned height)
{
  unsigned int i, p;
  for(i = py; i < py+ph; i++)
  {
...
void MCUVideoMixer::TopSplitLine(BYTE *d, unsigned px, unsigned py, unsigned pw, unsigned ph, unsigned width, unsigned height)
{
  unsigned int i, p;
  p = py*width+px;
  for(i = p; i < p+pw; i++)
...
void MCUVideoMixer::BottomSplitLine(BYTE *d, unsigned px, unsigned py, unsigned pw, unsigned ph, unsigned width, unsigned height)
{
  unsigned int i, p;
  p = (py+ph-1)*width+px;
  for(i = p; i < p+pw; i++)
...
BOOL MCUSimpleVideoMixer::ReadMixedFrame(VideoFrameStoreList & srcFrameStores, void * buffer, int width, int height, PINDEX & amount)
{
  ...
  for(unsigned i = 0; i < OpenMCU::vmcfg.vmconf[specialLayout].splitcfg.vidnum; i++)
  {
    ...
    MCUVMPList::shared_iterator it = FindVMP((int)i);
    if(it != vmpList.end())
    {
      VideoMixPosition *vmp = *it;
    ...
    {
      if(px != 0)
        LeftSplitLine((BYTE *)buffer, px, py, pw, ph, width, height);
      if(py != 0)
        TopSplitLine((BYTE *)buffer, px, py, pw, ph, width, height);
    }
  }

  // grid
  LeftSplitLine((BYTE *)buffer, 0, 0, width, height, width, height);
  RightSplitLine((BYTE *)buffer, 0, 0, width, height, width, height);
  TopSplitLine((BYTE *)buffer, 0, 0, width, height, width, height);
  BottomSplitLine((BYTE *)buffer, 0, 0, width, height, width, height);
...
{
  MCUVMPList::shared_iterator it = FindVMP(id);
  if(it == vmpList.end())
    return FALSE;
  VideoMixPosition *vmp = *it;
  vmp->offline = FALSE;
...
{
  VMPCfgOptions & o = OpenMCU::vmcfg.vmconf[specialLayout].vmpcfg[vmp->n];
  vmp->border = o.border;
  vmp->xpos = o.posx;
  vmp->ypos = o.posy;
  vmp->width = o.width;
...
MCUVMPList::shared_iterator MCUSimpleVideoMixer::VMPCreator(ConferenceMember * member, int n, int type)
{
  ...
  if(it == vmpList.end())
  {
    if(member != NULL) newPosition = CreateVideoMixPosition(member->GetID(), o.posx, o.posy, o.width, o.height);
    else newPosition = CreateVideoMixPosition(n, o.posx, o.posy, o.width, o.height);
...
{
  MCUVMPList::shared_iterator it = FindVMP(pos);
  if(it != vmpList.end())
  {
    it->type=type;
    return;
...
BOOL MCUSimpleVideoMixer::SetVAD2Position(ConferenceMember *member)
{
  ...
  MCUVMPList::shared_iterator old_vit = FindVMP(id);
...
void MCUSimpleVideoMixer::Exchange(int pos1, int pos2)
{
  ...
  MCUVMPList::shared_iterator it1 = FindVMP(pos1);
  MCUVMPList::shared_iterator it2 = FindVMP(pos2);
...
BOOL TestVideoMixer::AddVideoSource(ConferenceMemberId id, ConferenceMember & mbr)
{
  ...
  for (unsigned i = 0; i < _f; ++i) {
    ...
    { newPosition = CreateVideoMixPosition(i, o->posx, o->posy, o->width, o->height);
      newPosition->type=2;
    }
