...
//  var s="<div style='width:275px;height:17px;margin-bottom:5px'>"
  var s="<div style='width:"+panel_width+"px;height:"+PANEL_ITEM_HEIGHT+"px;margin-bottom:4px'>"
   +dpre+"0"+dmain+cp+"80px' onmousedown='if(checkcontrol())queue_otf_request("+OTFC_MUTE_ALL+")'>Mute&nbsp;All</div></div>"
...
//   +dpre+"56"+dmain+cm+"36px' id='rpan__2' name='rpan__2' onmousedown='ddstart(event,this,\"panel_top\",-2)'>VAD2</div></div>"
   +dpre+"102"+dmain+cp+"28px;background-color:#ccc' onclick='invite_all(this)'><span style='font-weight:bold;color:gray'><img width=15 height=15 alt='Inv.' src='i15_inv.gif'>*</span></div></div>"
   +dpre+"133"+dmain+cp+"28px;background-color:#ccc' onclick='remove_all0(this)'><span style='font-weight:bold;color:gray'><img width=15 height=15 alt='Rem.' src='openmcu.ru_remove.gif'>*</span></div></div>"
   +dpre+"171"+dmain+cp+"40px;background-color:#fcc' onclick='drop_all0(this)'><span style='font-weight:bold;color:red'><img width=15 height=15 alt='Drop' src='openmcu.ru_drop_Abdylas_Tynyshov.gif'>*</span></div></div>"
//   +dpre+"220"+dmain+cp+"50px;text-align:left;padding-left:4px' onclick='save_members_conf(this)'>Save</div></div>"
   +"</div>";
  return s;
}
...
//  alert(mmbr[1]); if(mmbr[1]!=0){alert('!');alert(mmbr[7]);}
 ...
 result='<div style="width:"+panel_width+"px" id="right_pan">'+additional_panel()+amr+vmr+imr+'</div>';
...
function build_page(){
  ...
  vmp_content=""; var pos_y=0; visible_ids=',';
//alert(array_print(conf));

// start mixers drawing

  for(i=0;i<mixers;i++){
    vmp_content+="<div id='mixercontrol"+i+"'>";
    var mw=conf[i+1][0][0]; var mh=conf[i+1][0][1]; var pos_x=(mmw-mw)>>1;

// walking through the mixer's positions

    for(var j=0;j<conf[i+1][1].length;j++){
      var id=false; try { if(typeof conf[i+1][2][j]!='undefined') id=conf[i+1][2][j]; } catch(e) {}; //alert(id + '\r\n\r\n' + array_print(conf[i+1]));
      if((id!=0)&&(id!=-1)&&(id!=-2)) visible_ids+=''+id+',';
      var type=false;
      if(id!==false) if(typeof conf[i+1][3][j]!='undefined') type=conf[i+1][3][j];
      ...
      var border='1px solid '+mix_color; if(id==-1)border='1px solid red'; else if(id==-2) border='1px dotted #f00';
      vmp_content+="<div id='pp"+i+"_"+j+"' style='position:relative;top:"+(pos_y+y)+"px;left:"+(pos_x+x)+"px;width:0px;height:0px'>"; // pointing block for member's mockup
        vmp_content+="<div"+ // rectangle for member's mockup
        ...
        var type_width=20; var user_width=w-type_width-clear_width; if(user_width<10)user_width=5;
        ...
        if(type===false)
        {
          ...
          vmp_content+="<div style='position:relative;top:0px;left:0px;width:0px;height:0px'>";
            vmp_content+="<div id='plus"+i+"_"+j+"'"
                        +" onmouseover='prvnt=1' onmouseout='prvnt=0' onclick='vmp_vad_click("+i+","+j+")'"
                        +" style='overflow:hidden;cursor:pointer;width:20px;height:"+(label_height-2)+"px'>";
              vmp_content+="<img src='i20_plus.gif' alt='Add video position' width=20 height=20>";
            vmp_content+="</div></div>";
        ...
        else
        {
          ...
          if (type_width>0)
          { vmp_content+="<div style='position:relative;top:0px;left:0px;width:0px;height:0px'>";
             vmp_content+="<div id='tp"+i+"_"+j+"'"
               +" onmouseover='prvnt=1' onmouseout='prvnt=0' onclick='vmp_vad_click("+i+","+j+","+type+")'"
               +" style='cursor:pointer;width:"+(type_width)+"px;height:"+(label_height-2)+"px'>";
              vmp_content+="<img src='i20_";
               if(type==2) vmp_content+='vad'; else if(type==3) vmp_content+='vad2'; else vmp_content+='static';
              vmp_content+=".gif' width="+type_width+" height="+label_height+">";
            vmp_content+="</div></div>";
          }

          if(user_width>0) if(type==1)
          { vmp_content+="<div style='position:relative;top:0px;left:"+(type_width)+"px;width:0px;height:0px'>";
             vmp_content+="<div id='us"+i+"_"+j+"'"
               +" onmouseout='javascript:{prvnt=0;}' onmouseover='javascript:{prvnt=1;}' onclick='javascript:{member_selector(this,"+i+","+j+","+id+")}'"
               +" style='x-overflow:hidden;white-space:nowrap;cursor:pointer;width:"+(user_width)+"px;height:"+(label_height-2)+"px'>";
              vmp_content+=member_read_by_id(id,2);
            vmp_content+="</div></div>";
          }

          if(clear_width>0)
          { vmp_content+="<div style='position:relative;top:0px;left:"+(type_width+user_width)+"px;width:0px;height:0px'>";
             vmp_content+="<div id='tp"+i+"_"+j+"'"
               +" onmouseover='prvnt=1' onmouseout='prvnt=0' onclick='if(checkcontrol()){queue_otf_request("+OTFC_REMOVE_VMP+","+i+","+j+");}'"
               +" style='cursor:pointer;width:"+(clear_width)+"px;height:"+(label_height-2)+"px'>";
              vmp_content+="<img src='i20_close.gif' width="+clear_width+" height="+label_height+">";
            vmp_content+="</div></div>";
          }
        }
    ...
    // mixer panel:
      ...
      vmp_content+="<div id='mxp"+i+"' style='position:relative;top:"+(pos_y+mh)+"px;left:0px;width:0px;height:0px'>"; // pointing block for mixer panel
        vmp_content+="<div onmouseover='mixer_over("+i+")' onmouseout='mixer_out("+i+")' style='width:"+(mmw-2)+"px;height:"+(MIXER_PANEL_HEIGHT-1)+"px;background-color:"+MIXER_PANEL_BGCOLOR+";overflow:hidden;text-align:left;line-height:"+(MIXER_PANEL_HEIGHT-1)+"px;padding-left:2px'>";
          vmp_content+="<span style='"+MIXER_PANEL_MIXER_STYLE+"'>#"+i;

          vmp_content+=" <span style='color:red;cursor:pointer;position:relative;left:0px' onclick='javascript:{if(checkcontrol())queue_otf_request("+OTFC_ADD_VIDEO_MIXER+","+i+");}'>+</span> ";
          if(mixers>1)vmp_content+=" / <span style='color:blue;cursor:pointer;position:relative;left:0px' onclick='javascript:{if(checkcontrol())queue_otf_request("+OTFC_DELETE_VIDEO_MIXER+","+i+");}'>&ndash;</span>";

          vmp_content+="</span>&nbsp;";
          vmp_content+=split_selector(i,conf[i+1][0][2]);
          ...
          vmp_content += str0 + "mix.gif' alt='Arrange members'" + str1 + OTFC_MIXER_ARRANGE_VMP + str2
...
//  main_content  = "<div style='text-align:center'>"; //try to center everything inside
//    main_content += "<div style='margin-left:auto;margin-right:auto;width:"+mmw+"px;height:"+total_height+"px;background-color:#fff'>"; // pointing block
      main_content = mockup_content;
//      main_content += log_content;
      main_content += workplace_content;
      main_content += panel_content;
      main_content += vmp_content;
//    main_content += "</div>"; // pointing
//  main_content += "</div>"; // centering
