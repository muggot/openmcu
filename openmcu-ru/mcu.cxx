...
#endif
  // internal rooms
  PString internalRoomPrefix = registrar->GetInternalRoomPrefix();
...
MCUURL::MCUURL(PString str)
{
  ...
  else if(partyUrl.Left(4) == "sips:") URLScheme = "sip";
  ...
  if(URLScheme == "sip")
  {
    ...
    if(partyUrl.Find("transport=tls") != P_MAX_INDEX) sipProto = "tls";
  }
}
...
PString convert_cp1251_to_utf8(PString text)
{
  ...
  if(text_utf != text)
  {
    PTRACE(5, "H225\tremotePartyName converted (cp1251->utf8): " << text << " -> " << text_utf);
    return text_utf;
  } else {
    return text;
  }
