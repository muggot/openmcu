...
window.l_connections_HIDDEN_PREFIX     = "<B>[Системна] </B>"  ;
...
<p>\
Конференція, створена підключенням першого терміналу, спочатку працює в некерованому режимі, при цьому всі учасники конференції бачать однакову картинку. У некерованому режимі OpenMCU-ru автоматично змінює розкладку екрану залежно від кількості підключених користувачів, у порядку, визначеному у файлі layouts.conf. Після підключення 100 користувачів всі наступні підключення користувачі будуть не видно на екранах терміналів, але будуть бачити перші 100 користувачів.<br />\
Звук в некерованому режимі передається і відтворюється від всіх підключених учасників.<br />\
</p>\
<p>\
...
<p>Нижче показана зведення по серверу.</p>\
";
...
<p>\
Цей логотип показується в мікшері при отсутсвии вхідного відео. Допускається завантаження тільки JPEG, не більше 500kB.\
</p>\
";

window.l_info_invite ="\
Для підключення в поле Room Name потрібно вказати ім'я конференції, \
до якої буде підключений термінал, в поле Address \
набрати IP адреса (або номер при використанні гейткипера) \
терміналу і натиснути кнопочку. \
...
window.l_name_display_name_override                = "Екранне ім'я";
