...
void MCUSimpleVideoMixer::Print_Subtitles(VideoMixPosition & vmp, void * buffer, unsigned int fw, unsigned int fh, unsigned int ft_properties){
  ...
  if(ft_properties & FT_P_REALTIME){ // rendering
    if(vmp.label_buffer.GetSize()<ft_width*ft_height*3/2){
      PTRACE(5,"FreeType\tSetting temp. vmp buffer size to " << (ft_width*ft_height*3/2));
...
void MCUSimpleVideoMixer::InsertVideoSource(ConferenceMemberId id, int pos)
{
  unsigned n=OpenMCU::vmcfg.vmconf[specialLayout].splitcfg.vidnum;
  ...
  if(oldPos!=-1) // remove from old vmp
  {
    ...
    if(id0!=NULL) for(int i=oldPos;i<(int)n-1;i++)
    {
      ...
      if(v==NULL) continue;
      NullRectangle(v->xpos,v->ypos,v->width,v->height);
      v->n=i;
  ...
  while(endPos>pos+1)
  {
    ...
    if(id3!=NULL)
    {
      ...
      if(id2==NULL)
      {
        NullRectangle(v3->xpos,v3->ypos,v3->width,v3->height);
        v3->n=endPos;
...
void MCUSimpleVideoMixer::PositionSetup(int pos, int type, ConferenceMemberId id) //types 1000, 1001, 1002, 1003 means type will not changed
{
  if((unsigned)pos>=OpenMCU::vmcfg.vmconf[specialLayout].splitcfg.vidnum) return; // n out of range

  PWaitAndSignal m(mutex);

  VideoMixPosition * old = VMPListFindVMP(id);
  if(old!=NULL)
  {
    if(old->n == pos)
    {
...
void MCUSimpleVideoMixer::Exchange(int pos1, int pos2)
{
  ...
  if(v1==NULL)
  {
    ...
    NullRectangle(v2->xpos, v2->ypos, v2->width, v2->height);
    v2->xpos=o.posx;
    v2->ypos=o.posy;
    v2->width=o.width;
    v2->height=o.height;
